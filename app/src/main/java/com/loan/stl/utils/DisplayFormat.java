...
public class DisplayFormat {
    ...
    public static String getResourcesString(int id) {
        return LoanApplication.getContext().getString(id);
    }
    ...
    public static Spannable XLIFFAprFormat(String params, int size) {
        ...
        if (params.contains(".")) {
            ...
            // 设置字体大小
            span.setSpan(new AbsoluteSizeSpan(DensityUtil.sp2px(LoanApplication.getContext(), size)), start, end, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
            return span;
        } else {
    ...
    public static Drawable getBackground(Drawable drawable){
        drawable.setColorFilter(ContextCompat.getColor(LoanApplication.getContext(), R.color.app_base_color), PorterDuff.Mode.MULTIPLY);
        return drawable;
    }
}
